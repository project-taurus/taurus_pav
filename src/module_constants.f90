...
integer, parameter :: uti = input_unit,  &
                      uto = output_unit, &
                      utlw = uti + uto + 1, &  ! left  wave function 
                      utrw = uti + uto + 2, &  ! right wave function 
                      uth = uti + uto + 3, &   ! hamiltonian (main)
                      uth1 = uth + 1, &        !     "       (1b)
                      uth2 = uth + 2, &        !     "       (2b)
                      uthc = uth + 4, &        !     "       (2b com)
                      uthr = uth + 5, &        !     "       (red)
                      utro = uti + uto + 20, & ! rotated matrix elements 
                      utst = uti + uto + 21, & ! proj mat elem : states 
                      utoc = utst + 1, &       !   "   "   "   : occ. numbers
                      ute1 = utst + 2, &       !   "   "   "   : E1
                      ute2 = utst + 3, &       !   "   "   "   : E2
                      ute3 = utst + 4, &       !   "   "   "   : E3
                      utm1 = utst + 5, &       !   "   "   "   : M1
